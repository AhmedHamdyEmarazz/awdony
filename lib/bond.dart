...
class _BondState extends State<Bond> {
  ...
  currentAya(String currentaya, ScrollController controller, suranamee) {
    ...
    // var targSura = alllis.firstWhere(
    //     (element) => (element!.firstWhere((element) => element == currentaya)));
    // var suralength = targSura!.length;
    var currentMap =
        allquraan.firstWhere((element) => element.containsKey(suranamee));

    // print(currentSuraList!.length);
...
// allquraan[currentMapIndex]
//         .values
//         .toList()
//         .indexWhere((element) => element == currentaya);
    // print(currentMap);
    // print(currentMapIndex);
    //  print((suralength) - myAyaindex);
    for (int i = 0; i < currentSuraList!.length - myAyaindex; i++) {
      setState(() {
        _counterx++;
        //   print(_counterx);
      });
      // var nextsuraindex = allquraan[currentMapIndex + 1].keys.firstWhere(
      //     (element) => element.characters.contains('(${myAyaindex + 1})'));
      var nextaya = allquraan[currentMapIndex].values.first!.firstWhere(
          (element) => element.toString().contains('(${myAyaindex + 1})'));
      // print(myAyaindex);
      // print(currentaya);
      ...
      //  var nextaya = ayamap.values.toList()[myAyaindex + 1 + i].toString();
